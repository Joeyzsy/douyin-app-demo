...
func Feed(c *gin.Context) {
	...
	//获取所需要的VideoList
	numVideos, err := service.GetvideoAndAuthor(&videoList, &authorList, LatestTime, global.FEED_NUM)
	if err != nil {
		// 访问数据库出错
	...
	fmt.Printf("numVideos=%d\n", numVideos)

	if numVideos == 0 {
		// 没有满足条件的视频 使用当前时间再获取一遍
		numVideos, _ = service.GetvideoAndAuthor(&videoList, &authorList, CurrentTimeInt, global.FEED_NUM)
		if numVideos == 0 {
			// 后端没有视频了
